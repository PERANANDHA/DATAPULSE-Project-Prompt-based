...
const createWordDocument = async (
  ...
  if (calculationMode === 'sgpa' || (calculationMode === 'cgpa' && currentSemesterRecords.length > 0)) {
    // FIXED: Explicitly filter out arrear subjects for End Semester Analysis
    // This is the key change to fix the issue reported by the user
    const recordsForEndSemesterAnalysis = currentSemesterRecords.filter(record => !record.isArrear);
    
    console.log(`End Semester Analysis: Using ${recordsForEndSemesterAnalysis.length} records (excluded ${currentSemesterRecords.length - recordsForEndSemesterAnalysis.length} arrear subjects)`);
    
    // Use non-arrear subjects for analysis in both SGPA and CGPA modes
    ...
    // Adding actual subject data with faculty names - using only non-arrear subjects
    uniqueSubjects.forEach((subject, index) => {
      const subjectRecords = recordsForEndSemesterAnalysis.filter(record => record.SCODE === subject);
