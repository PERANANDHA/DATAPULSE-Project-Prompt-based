...
// Calculate CGPA including arrear subjects from multiple semesters (files)
export const calculateCGPAWithArrears = (
  ...
  const allSemesters = Object.keys(fileGroups);
  if (allSemesters.length <= 1) {
    // If only one semester, calculate including arrear subjects
    return calculateSGPAWithArrears(records, studentId);
  }
  ...
  allSemesters.forEach(semester => {
    const semesterRecords = fileGroups[semester];
    // Include all subjects, even arrear subjects
    const studentSemRecords = semesterRecords.filter(record => 
      record.REGNO === studentId
      // Include arrear subjects
    );
...
// Calculate SGPA including arrear subjects
export const calculateSGPAWithArrears = (records: StudentRecord[], studentId: string): number => {
  // Get all records for this student, including arrear subjects
